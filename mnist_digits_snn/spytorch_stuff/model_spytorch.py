...
class SNNModel():
    ...
    def load_weights(self, weights_matrix):
        self.v1 = torch.Tensor(weights_matrix, device=self.device, dtype=self.dtype, requires_grad=True)
    ...
    def train(self, input_tensor, lr=1e-3, num_epochs=10, save_weights_path=None):
        ...
        for e in range(num_epochs):
            ...
            for batch_i in tqdm(range(self.num_batches), f'epoch {e+1}'):
                ...
                # batch_loss = self.L1_loss_function(mem_rec) # preactivation (membrane voltage) loss
                batch_loss = self.L1_loss_function(spk_rec) # postactivation (spike) loss
